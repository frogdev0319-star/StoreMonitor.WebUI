<template>
  <el-row class="el-device" :style="{'min-height':varyWindowHeight-139+'px'}">
    <el-col :span="24" class="el-btns">
      <div style="display: inline-block;position:absolute;z-index: 979;right: 30px;top: 23px;float: right;">
        ...
        <el-button v-for="(item,index) in btnList"
                   :key="index" size="mini" @click="handleBeseye(index,item)" :class="lang=='en'? 'en-el-handle-btn': 'el-handle-btn'" :disabled="index==2">
          <div class="btn-area">
    ...
    <el-col :span="24" class="el-tabPanels">
      <el-tabs v-model="activeName" id="en-devicetabs-content">
        <el-tab-pane :label="$t('deviceView.videoMangement')" name="beseyeVideo">
          <el-col :span="lang=='en' && varWindowWidth<1920? 9: 10" class="lisde">
            <div class="nvr-info">
              ...
              <el-button type="primary" size="mini" class="add-btn"
                         @click="showAddDialog">
                <i style="margin-right:8px;" class="iconfont el-icon-plus"></i><span>{{$t('deviceView.addDevice')}}</span>
              </el-button>
            </div>
            ...
            <el-scrollbar style="height:100%;" id="el-menuscrollbar">
              <div  :style="{'max-height':varyDivHeight+'px','min-height':varyDivHeight+'px'}">
                <div class="nvr-data group-title"
                     ...
                     :key="index"  :class="!item.isClick?'noraml-color':'active-color'" @click="clickBeseye(index,item)">
                ...
                <el-dialog  :title="generateDeviceLang('prompt')"
                  ...
                  <div slot="footer" class="dialog-footer">
                    ...
                    <el-button class="file-confirm-btn" @click="deleteSingleBeseye()" size="mini" type="primary">{{generateDeviceLang('confirm')}}</el-button>
                  </div>
                </el-dialog>
              </div>
            </el-scrollbar>
          ...
          <el-dialog :title="$t('deviceView.addDevice')"
                     :visible.sync="showAddBeseyeDialog" v-if="showAddBeseyeDialog"
                     :append-to-body='true'
                     :close-on-click-modal="false"
                     width="510px"
                     top="35vh"
                     left="40vh" customClass="addBeseye"
          >
            <div class="dialog-content" style="overflow:hidden;width:100%;">
              <hr style="border: 0.5px solid #dfe2e9;"/>
              <el-form :model="addBeseyeData" :rules="rules" ref="beseyeForm" class="beseyeForm" label-position="top" size="mini">
                <el-form-item label="Camera ID" prop="serialNumber">
                  <el-input v-model="addBeseyeData.serialNumber" @input="(val)=>cameraIdChange(val)" @blur="notShowInputRuleTips('cameraId')"></el-input>
                  <span class="rules" v-if="cameraIdRuletip">{{$t('scheduleView.scheduleNameRuletip')}}</span>
                </el-form-item>
                <el-form-item style="height: 57px;">
                  <el-col :span="13">
                    <el-form-item prop="name" :label="$t('deviceView.deviceName')" style="margin-bottom:0;">
                      <el-input v-model="addBeseyeData.name" style="width: 100%;" @input="(val)=>nvrNameChange(val, {})" @blur="notShowInputRuleTips('Nvrname')"></el-input>
                  ...
                  <el-col :span="9" :offset="2">
                    <el-form-item prop="channelCount" :label="generateDeviceLang('channelNum')">
                      <el-select v-model="addBeseyeData.channelCount" size="mini">
                        <el-option
                ...
                <el-form-item :label="generateDeviceLang('store')" prop="storeId">
                  <el-select v-model="addBeseyeData.storeId" style="width: 100%;">
                    <el-option
            ...
            <div slot="footer" class="dialog-footer">
              <el-button class="file-cancel-btn" @click="cancelAddBeseye" size="mini" style="">{{generateDeviceLang('cancle')}}</el-button>
              <el-button class="file-confirm-btn" @click="addSingleBeseye" size="mini" type="primary">{{generateDeviceLang('confirm')}}</el-button>
            </div>
          </el-dialog>
          <el-col :span="lang=='en' && varWindowWidth<1920? 15: 14" class="risde">
            <div class="nvr-info">
              ...
              <el-button type="primary" size="mini" class="add-btn"
                ...
                <i style="margin-right:8px;" class="iconfont el-icon-plus"></i><span>{{$t('deviceView.addChannel')}}</span>
              </el-button>
            </div>
            ...
            >
              <div class="dialog-content" style="overflow:hidden;width:100%;">
                <hr style="border: 0.5px solid #dfe2e9;"/>
                <el-form :model="addChannelData" :rules="channelRules" ref="channelForm" class="beseyeForm" label-position="top" size="mini">
                  <el-form-item style="height: 57px;">
                    <el-col :span="12">
                    ...
                    <el-col :span="8">
                      <el-form-item :label="generateDeviceLang('thumbnail')" ref="uploadElement" prop="pictureUrl">
                        ...
                        <el-upload
                          ...
                          <!--<img :src="addChannelData.pictureUrl" class="avatar" style="border: 1px beseyeed #d9d9d9;">-->
                        </el-upload>
...
<script>
  ...
  import {addBeseyeDevice, deleteBeseyeDevice, getBeseyeList, updateBeseyeDevice} from "../../../api/baseye";
  ...
  export default {
    ...
    methods:{
      ...
      clickBeseye(index,item){
        ...
        self.getChannelListByNVR(item.serialNumber);
      ...
      checkDashInfo(){
        let self=this;
        let msg='';
        if(self.beseye.url.toString().trim().length==0||self.beseye.httpCmdPort.toString().trim().length==0
          ||self.beseye.httpsCmdPort.toString().trim().length==0||self.beseye.dataPort.toString().trim().length==0){
          //msg='当前配置项均为必填项！';
          msg=this.$t('deviceView.emptyInfo');
        }
        if(!validateURL(self.beseye.url)){
          //msg='当前配置项中IP格式错误！';
          msg=this.$t('deviceView.ipErrorInfo');
        }
        if(validatePort(self.beseye.httpCmdPort)
          ||validatePort(self.beseye.httpsCmdPort)||validatePort(self.beseye.dataPort)){
          //msg='当前配置项端口格式错误！';
      ...
      getDashURL(){
        ...
        let url='';
        let dataPort='';
        if(self.beseye.url.indexOf('https')!=-1){
          dataPort=self.beseye.httpsCmdPort;
        }
        else{
          dataPort=self.beseye.httpCmdPort;
        }
        url=self.beseye.url+':'+dataPort;
        return url;
      },
      async getChannelListInit(){
        ...
        self.getBeseyeList(params);       //获取Beseye数据信息
      },
      handleClick(tabs){
        console.log(tabs);
        let self=this;
      ...
      async addAllData(paramsNVR,paramsDevice){
        ...
        let res1= await self.addBeseye(paramsNVR);
        ...
        FileReader.prototype.readAsBinaryString = function(f) {
          ...
          reader.onload = async function(e) {
            ...
            arr.forEach((item,index)=>{
              if(beseyeDataTemp.map(x=>x.cameraId).indexOf(item["IVS ID"])==-1){
                let obj={
                  cameraId:item["IVS ID"],
                  name:item["NVR名称"],
                  channelCount:item["通道数"],
                  storeId:item["StoreID"]
                }
                beseyeDataTemp.push(obj);
              }
              let obj={
                name:item["通道名称"],
                storeId:item["StoreID"],
      ...
      export2Excel() {
        ...
        require.ensure([], async() => {
          ...
          const tHeader = ['StoreID','所属门店', 'Camera ID','设备名称','通道数','通道名称','通道序号']; // 导出的表头名
          const filterVal = ['storeId','storeName','serialNumber','deviceName','channelCount','channelName','channelNum']; // 导出的表头字段名
          console.log(that.activeName);
          let beseyeData=await that.getAllBeseyeData();
          let channelData=that.channelData;
          let excelData=[];
          if(beseyeData.length!=0&&channelData.length!=0){
            beseyeData.forEach((item,index)=>{
        ...
        if(item.id != 0){
          ...
          deviceRESTful.updateDevice(params).then( async res=>{
            ...
            .then(async()=>{
              ...
              self.getChannelListByNVR(self.curBeseyeItem.serialNumber);
            })
        }
        else{
          //新增通道
          ...
          json.storeId = self.curBeseyeItem.storeId;
          json.cameraId = self.curBeseyeItem.serialNumber;
        ...
        getBeseyeList(params).then(res=>{
          ...
          if(errCode == 0){
            ...
            data.forEach((item,index)=>{
              ...
              obj.storeId=item.storeId;
              obj.channelCount = 1;
              obj.tempChannelCount = 1;
              obj.channelNum= 1 + self.$t('deviceView.unit');
              obj.isEditing = false;
              if(index==0){
                obj.isClick=true;
              }
            ...
            self.beseyeData=temp;
            self.total=res.data.totalElements;
          }
        })
          .then(async()=>{
            if(self.beseyeData.length!=0){
              console.log(self.beseyeData[0].serialNumber);
              self.curBeseyeItem=self.beseyeData[0];
              self.channelData=await self.getChannelData();
              console.log(self.channelData)
              self.getChannelListByNVR(self.beseyeData[0].serialNumber);
            }
            else{
              self.channelData=[];
              self.channelList=[];
      ...
      getChannelListByNVR(cameraId){
        let self=this;
        let temp=[];
        self.channelData.forEach(item=>{
          if(item.ivsId==cameraId){
            let obj={};
            obj.id=item.id;
            obj.name=item.name;
        ...
        console.log(self.channelList);
        if(self.channelList.length == self.curBeseyeItem.channelCount){
      ...
      addSingleBeseye(){
        ...
        self.$refs['beseyeForm'].validate(async (valid) => {
          if (valid) {
            ...
            let res1= await self.addBeseye(beseyeParams);
      ...
      addSingleChannel(){
        ...
        self.$refs['channelForm'].validate(async (valid) => {
          if (valid) {
            ...
            json.storeId = self.curBeseyeItem.storeId;
            json.ivsId = self.curBeseyeItem.serialNumber;
      ...
      confirmEditBeseye(index,item){
        ...
        // params.channelCount = item.tempChannelCount;
      ...
      //删除单个Beseye
      async deleteSingleBeseye(){
        let self = this;
        self.showConfirmDelete = false;
        let serialNumber = self.curBeseyeItem.serialNumber;
        let json = {};
        ...
        if(channelList.length > 0 ){
          ...
          let res2 = await self.deleteBeseye(json);
          if(res1.errMsg=='Success'&&res2.errMsg=='Success'){
            self.notify(self.$t('deviceView.deleteSuccess'),'success',3000);
          }
          else{
      ...
      addNewChannel(){
        let self = this;
        let channelNum = self.curBeseyeItem.channelCount;
      ...
      cameraIdChange(val){
        ...
        console.log(comment);
        self.addBeseyeData.serialNumber = comment;
        let length = filterString.getContentLength(val);
        if(length>30){
          this.cameraIdRuletip=true
        }else{
          this.cameraIdRuletip=false
        }
      },
